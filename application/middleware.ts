...
class ApplicationMiddleware implements Application, DataSyncSubscriber {
    ...
    private constructor(options?: ApplicationMiddlewareOptions) {
        ...
        if (options?.contentImportPath) {
            ...
            this.#contentImportPath = Deno.env.get(applicationContentOriginPathVariable);
        }
        ...
        if (this.#contentImportPath) {
            this.#dataManager.addSyncRule('content', { sourcePath: this.#contentImportPath }, this);
        } else {
            this.#dataManager.storeExists('content').then(exists => {
                if (exists) {
                    this.updated();
                } else {
                    throw new Error(`application content origin path not set.\n\nTry export=${applicationContentOriginPathVariable}=<content path>`);
                }
            });
        }
    }
	...
	updated = async () => {
        const contentStoreDirectory = await this.#dataManager.getStore('content');

        if (this.#manifestPath) {
            await this.#loadManifest(this.#manifestPath);
        }
        
        await this.#loadNavigation(resolve(contentStoreDirectory, ApplicationMiddleware.#navigationPath));
    ...
    #loadNavigation = async (path: string) => {
        ...
        for await (const content of navigationDirectory) {
            if (content.isFile) {                
                ...
                const navigationFetchResponse = await fetch(new URL(navigationFilePath, import.meta.url));
                const navigationFileContent: NavigationItem[] = await navigationFetchResponse.json();
                this.navigation.sections[basename(content.name, extname(content.name))] = navigationFileContent;
            }
        }
    }
}
...
export {
    type ApplicationMiddlewareOptions,
